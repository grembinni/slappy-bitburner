...
async function runBigServers(ns, serverRefs) {
	...
	var threads = await calculateThreadsPerInstance(ns, 'home', 'run-hack-seq.js', 1, bigUsage);
	...
	for (const serverRef of serverRefs) {
		await runThreaded(ns, 'home', 'run-hack-seq.js', threads, [serverRef.serverName]);
	}
}
...
async function runMediumServers(ns, serverRefs) {
	...
	var threads = await calculateThreadsPerInstance(ns, 'home', 'run-hack-seq.js', 1, mediumUsage);
...
async function runSmallServers(ns, serverRefs) {
	...
	var threads = await calculateThreadsPerInstance(ns, 'home', 'run-hack-seq.js', 1, smallUsage);
